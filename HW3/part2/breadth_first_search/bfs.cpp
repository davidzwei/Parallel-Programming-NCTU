...
// swap pointers
// frontier = new_frontier, reset new_frontier in next iteration
#pragma omp single
        {
    ...
    int *distances)
{
    ...
    int cur_distance = distances[frontier->vertices[0]]; // current distance in frontier
        ...
        for (int i = 0; i < g->num_nodes; i++)
        {
            ...
            if (distances[i] == NOT_VISITED_MARKER)
            {
                ...
                for (int neighbor = start_edge; neighbor < end_edge; neighbor++)
                {
                    ...
                    // check if in frontier equals
                    // check distance incoming == frontier distance
